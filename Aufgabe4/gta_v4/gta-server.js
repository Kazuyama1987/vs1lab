 ...
 */
 ...
 app.use(express.static(__dirname + "/public"));        //_dirtname = current folder /public
 ...
 var geotagModule = (function (){
     ...
     return{
         ...
         searchGeotagsRadius : function (latitude, longitude) {
             ...
             var tagsFound = [];
             geoTags.forEach(function (tag){                                    //For-Each iterats throught the array
                 if(radius >= Math.sqrt(Math.pow(tag.latitude - latitude, 2)    //figure out the radius of a point with SAtz des Phytagoras
                     + Math.pow(tag.longitude - longitude, 2))){
                     tagsFound.push(tag);                                       //If Yes push it in the tagsFound
                 }
             });
             return tagsFound;                                                  //return all points in the radius
         },
         //Funktion zur Suche von Geo Tags nach Suchbegriff
         searchGeotags : function (search){
             var tagsFound = [];
             geoTags.forEach(function (tag){
                 if(tag.name.toString().includes(search.toString())){           //convert the name of the tag to String and compare ist to search
                     tagsFound.push(tag);
                 }
             });
 ...
 app.post('/tagging', function (req,res){
     ...
     var tagInRadius = geotagModule.searchGeotagsRadius(req.body.latitude, req.body.latitude);          //add geotag with body-informations
 ...
 app.post('/discovery', function (req,res){
     var tagInRadius = geotagModule.searchGeotagsRadius(req.body.latitude, req.body.latitude);  //search geotag
